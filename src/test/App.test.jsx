...
describe('Book Store', () => {
    ...
    test("Show price with discount when adding four books", async () => {
        await addGivenBooksToBasket([1, 2, 3, 4])
        expect(screen.getByTestId(CONSTANTS.TEST_ID_TOTAL)).toHaveTextContent(CONSTANTS.FOUR_BOOK_WITH_DISCOUNT_TOTAL)
        expect(screen.getByTestId(CONSTANTS.TEST_ID_SUBTOTAL)).toHaveTextContent(CONSTANTS.FOUR_BOOK_WITH_DISCOUNT_SUBTOTAL)
        expect(screen.getByTestId(CONSTANTS.TEST_ID_DISCOUNT)).toHaveTextContent(CONSTANTS.FOUR_BOOK_WITH_DISCOUNT_DISCOUNT)
    })
    test("Show price with discount when adding five books", async () => {
        await addGivenBooksToBasket([0, 1, 2, 3, 4])
        expect(screen.getByTestId(CONSTANTS.TEST_ID_TOTAL)).toHaveTextContent(CONSTANTS.FIVE_BOOK_WITH_DISCOUNT_TOTAL)
        expect(screen.getByTestId(CONSTANTS.TEST_ID_SUBTOTAL)).toHaveTextContent(CONSTANTS.FIVE_BOOK_WITH_DISCOUNT_SUBTOTAL)
        expect(screen.getByTestId(CONSTANTS.TEST_ID_DISCOUNT)).toHaveTextContent(CONSTANTS.FIVE_BOOK_WITH_DISCOUNT_DISCOUNT)
    })
    test("Display quantities of books and pricing summary when adding more than one sets of books", async () => {
        await addGivenBooksToBasket([0, 1, 2, 3, 4, 1, 2])
        expect(screen.getByTestId(CONSTANTS.TEST_ID_TOTAL)).toHaveTextContent(CONSTANTS.SEVEN_BOOK_WITH_DISCOUNT_TOTAL)
        ...
        expect(screen.getAllByText(CONSTANTS.BOOK_QUANTITY_SEPARATOR + '1')).toHaveLength(3)
        expect(screen.getAllByText(CONSTANTS.BOOK_QUANTITY_SEPARATOR + '2')).toHaveLength(2)
    })
    test("Show price with discount when adding three sets of books", async () => {
    ...
    test("Update basket item quantity when adding and removing copies", async () => {
        const addBookToBasket = [1]
        await addGivenBooksToBasket(addBookToBasket)
        ...
        expect(screen.getByText(CONSTANTS.BOOK_QUANTITY_SEPARATOR + (CONSTANTS.QUANTITY_STEP))).toBeInTheDocument()
        expect(screen.getAllByTestId(CONSTANTS.TEST_ID_BASKET_ITEM)).toHaveLength(CONSTANTS.QUANTITY_STEP)
        await userEvent.click(removeSingleButton)
        expect(screen.getByText(CONSTANTS.BASKET_EMPTY_MESSAGE)).toBeInTheDocument()
    })
})